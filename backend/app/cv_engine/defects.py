...
def compute_defect_metrics(bgr_img: np.ndarray, gray_img: np.ndarray) -> dict:
    """
    Detects visual defects such as:
    - Scratches & line defects (Hough Lines / Morphological TopHat)
    - Spot & dust anomalies (Blob / Contour detection on local anomaly map)
    - Blotches / local stains (Color/Luminance deviation from neighborhood)
    Returns defect area ratio, count of detected defect regions, localized mask, and defect score.
    """
    ...
    # 1. Scratch & Line Detection (Top-Hat Morphological Filter + Canny)
    ...
    for cnt in scratch_contours:
        ...
        if area > 10:
            # Check aspect ratio / eccentricity for thin scratch shape
    ...
    # 2. Spot / Dust / Stain Detection (Local Std Dev Anomaly Map)
    ...
    # Defect Quality Score (100 = flawless, lower = defective)
